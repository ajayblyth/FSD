...
2. Insert two students in a single command: Kavya (Mumbai, Science, 81) and Manish (Pune, Math, 74)
...
9. Find all students but show only the name and marks fields, hiding the _id
...
for delhi students:
db.students.find({city: 'Delhi', subject: { $in: ['Math', 'English'] }
})
...
24. Show only the name and salary of all employees, hiding the _id
...
27. Find students who are in Delhi or Mumbai, using a single list operator

school> db.students.find({city: {$in:['Delhi', 'Mumbai']}})

28. Find students who are NOT in Delhi or Mumbai, using a single list operator
...
31. Find students whose name contains 'ar', ignoring case
school> db.students.find({name: {$regex: 'ar', $options: 'i'}})

32. Using a logical operator, find students in Delhi whose marks are at least 85
...
33. Using a logical operator, find students whose marks are above 95 or below 70

school> db.students.find({ $or: [{marks: {$gt:95}},{marks: {$lt:70}}]})


34. Find students whose marks are NOT greater than 80 (use a negation operator)

school> db.students.find({marks: {$not: {$gt:80}}})


35. Find students who are neither in Delhi nor have marks equal to 90
...
db.students.aggregate([{ $project: { name:1, marks:1, grade: 
{ $cond: [{ $gte:['$marks',90] }, 'A', 'B'] } } }])


48. Add a field 'grade': 'A' for 90+, 'B' for 80 to 89,
    otherwise 'C'

db.students.aggregate([{ $project: { name:1, marks:1, grade: { $cond: [{ $gte:['$marks',90] }, 'A', { $cond: [{ $gte:['$marks',80] }, 'B', 'C'] } ] } } }])
...
school> db.students.aggregate([ {$sort: {marks: 1}}, {$limit:2}])
...
company> db.employees.find({
  $or: [{ department: 'Engineering' },{ salary: { $gt: 100000 } }]})


56. Find students whose marks are one of 85, 90, or 95

school> db.students.find({marks: { $in: [85, 90, 95] }
})
...
db.students.find({subject: 'Math',marks: { $gte: 80 }})
...
{$unwind:'$courseDetails'},
{$addFields:{course:'$courseDetails.course',fees:'$courseDetails.fees',grade:{$cond:{if:{$gte:['$marks',90]},then:'A',else:{$cond:{if:{$gte:['$marks',80]},then:'B',else:{$cond:{if:{$gte:['$marks',70]},then:'C',else:'D'}}}}}}}},
...
{$sort:{avgMarks:-1}},
{$project:{_id:0,city:'$_id',avgMarks:1,maxMarks:1,minMarks:1,totalStudents:1,totalFees:1,students:1}},
{$out:'finalReport'}
])
...
Doubt:In $project, values are not limited to 0 and 1.

1  -> include existing field
0  -> exclude field
'$fieldName' -> create/rename field using value from another field
...
75. Run explain on salary query before any index exists.
...
totalDocsExamined:All documents in collection

76. Create compound index on city ascending and marks descending.
...
77. Create unique index on student name and insert duplicate.
...
78. Create text index on name and city, then search Delhi.
...
79. List all indexes, then drop department index.

company> db.employees.getIndexes()

company> db.employees.dropIndex({department: 1})


80. A query filters by city and sorts by salary.
    Best index?
...
- avoids collection scan and in-memory sort

Benefit:
MongoDB can:
Quickly narrow rows by city
Already have results sorted by salary
